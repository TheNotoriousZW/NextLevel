...
def update_targets(table, time):
  ...
  try:
    ...
    for target in targets:
      ...
      if time_difference >= timedelta(days=time) and target.completed:
      ...
      elif time_difference >= timedelta(days=time) and not target.completed:
        now = datetime.now()
        sql_timestamp = now.strftime('%Y-%m-%d %H:%M:%S.%f')
...
def scheduler(task, *args):
  schedule.every(30).seconds.do(task,*args)

  while True:
    schedule.run_pending()
    time.sleep(1)

dtarget_set = threading.Thread(target=scheduler, args=(update_targets,models.Dailytargets, 1))
dtarget_set.start()

ptarget_set = threading.Thread(target=scheduler, args=(update_targets,models.Proactivetargets, 7))
ptarget_set.start()

ytarget_set = threading.Thread(target=scheduler, args=(update_targets,models.Yearlytargets, 365))
ytarget_set.start()
...
origins = [
    ...
    "http://localhost:3000",
    "*",
    "http://52.15.219.236:3000",
    "http://52.15.219.236:8000",
    "http://52.15.219.236:5432"
]
