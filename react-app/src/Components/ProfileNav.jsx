...
useEffect(() => {
  ...
  return (
    <motion.div className="flex h-20 md:rounded-lg text-white md:p-2 px-24 w-auto md:w-auto md:m-5 ring-sky-200/50 shadow-md bg-gradient-to-r from-black  to-purple-600/40 bg-opacity-25 ring-1 ring-sky-200 md:justify-center items-center md:gap-20 gap-8 ">
     ...
     <motion.div className="flex md:gap-2 font-bold items-center  ">
      <motion.p  animate={{x: 0, duration: 3}} transition={{type: "spring", stiffness: "250"}} className="text-white text-sm p-1 mr-0 ml-1 font-serif rounded-md md:p-2 ">Honors: <motion.span initial={{}} className="self-center text-white">{props.points}</motion.span></motion.p>
     </motion.div>
     ...
     <motion.div drag="x" dragConstraints={{left: 50, right: 300}} dragElastic={0.5} className="w-30 h-10 self-center flex shrink-[4] shadow-sm rounded-xl">
      <motion.img initial={{scale: 1, x: -10}}src={props.userIcon} className="rounded-full ring-1 ring-sky-100 md:h-12 md:w-12 h-10 w-10 ml-2 shadow-sky-200 "/>
